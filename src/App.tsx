...
import { Mic, MicOff, Loader2, Volume2, VolumeX, Keyboard, Send, Trash2, Compass, Terminal } from "lucide-react";
...
export default function App() {
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    // 1. Check for browser or system commands
    ...
    if (commandResult.systemAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-n", sender: "nema", text: responseText }]);
      ...
      responseText = await getNemaResponse(finalTranscript, messagesRef.current);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-n", sender: "nema", text: responseText }]);
